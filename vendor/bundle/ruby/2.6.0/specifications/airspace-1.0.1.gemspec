# -*- encoding: utf-8 -*-
# stub: airspace 1.0.1 ruby lib

Gem::Specification.new do |s|
  s.name = "airspace".freeze
  s.version = "1.0.1"

  s.required_rubygems_version = Gem::Requirement.new(">= 0".freeze) if s.respond_to? :required_rubygems_version=
  s.require_paths = ["lib".freeze]
  s.authors = ["Matthew Ruggio".freeze]
  s.date = "2019-02-22"
  s.description = "    This library provides a very simple interface for storing/fetching/paging datasets in Redis.\n".freeze
  s.email = ["mruggio@bluemarblepayroll.com".freeze]
  s.executables = ["console".freeze]
  s.files = ["bin/console".freeze]
  s.homepage = "https://github.com/bluemarblepayroll/airspace".freeze
  s.licenses = ["MIT".freeze]
  s.required_ruby_version = Gem::Requirement.new(">= 2.3.8".freeze)
  s.rubygems_version = "3.0.3".freeze
  s.summary = "Redis Dataset Store".freeze

  s.installed_by_version = "3.0.3" if s.respond_to? :installed_by_version

  if s.respond_to? :specification_version then
    s.specification_version = 4

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<redis>.freeze, [">= 3.3.0"])
      s.add_development_dependency(%q<database_cleaner>.freeze, ["~> 1.7"])
      s.add_development_dependency(%q<guard-rspec>.freeze, ["~> 4.7"])
      s.add_development_dependency(%q<pry>.freeze, ["~> 0.12"])
      s.add_development_dependency(%q<rspec>.freeze, ["~> 3.8"])
      s.add_development_dependency(%q<rubocop>.freeze, ["~> 0.63.1"])
      s.add_development_dependency(%q<simplecov>.freeze, ["~> 0.16.1"])
      s.add_development_dependency(%q<simplecov-console>.freeze, ["~> 0.4.2"])
    else
      s.add_dependency(%q<redis>.freeze, [">= 3.3.0"])
      s.add_dependency(%q<database_cleaner>.freeze, ["~> 1.7"])
      s.add_dependency(%q<guard-rspec>.freeze, ["~> 4.7"])
      s.add_dependency(%q<pry>.freeze, ["~> 0.12"])
      s.add_dependency(%q<rspec>.freeze, ["~> 3.8"])
      s.add_dependency(%q<rubocop>.freeze, ["~> 0.63.1"])
      s.add_dependency(%q<simplecov>.freeze, ["~> 0.16.1"])
      s.add_dependency(%q<simplecov-console>.freeze, ["~> 0.4.2"])
    end
  else
    s.add_dependency(%q<redis>.freeze, [">= 3.3.0"])
    s.add_dependency(%q<database_cleaner>.freeze, ["~> 1.7"])
    s.add_dependency(%q<guard-rspec>.freeze, ["~> 4.7"])
    s.add_dependency(%q<pry>.freeze, ["~> 0.12"])
    s.add_dependency(%q<rspec>.freeze, ["~> 3.8"])
    s.add_dependency(%q<rubocop>.freeze, ["~> 0.63.1"])
    s.add_dependency(%q<simplecov>.freeze, ["~> 0.16.1"])
    s.add_dependency(%q<simplecov-console>.freeze, ["~> 0.4.2"])
  end
end
